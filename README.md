...
**AI에게 설치부터 화면 검증까지 맡기는 Windows용 스킬.**

VS Code를 **미리보기 | 코드 | OpenCode**로 배치합니다. `F12`를 누르면 전체 터미널로, 다시 누르면 3열로 돌아옵니다.

**[친절한 시작 안내 →](https://arthong1.github.io/vsstudiovibecoding/)** · **[스킬 ZIP 다운로드](https://arthong1.github.io/vsstudiovibecoding/downloads/vibe-coding-1.2.0.zip)** · [문제 신고](https://github.com/ARTHONG1/vsstudiovibecoding/issues)
...
필수 확장, 미리보기 | 코드 | OpenCode 3열 화면과 F12 전환까지 설정해줘.
기존 설정과 파일은 보존하고 실제 화면, 버튼, F12 왕복,
...
3. `F12` → 전체 터미널 → `F12` → 3열 복귀를 확인합니다. 화면 전환이 끝날 때까지 잠깐 기다리세요.
4. `Ctrl+Alt+V`로 배치를 복원해도 미리보기와 터미널이 중복되지 않아야 합니다.
5. 생성된 **Vibe Coding - 프로젝트명-식별자** 바탕화면 바로가기로 다시 열어도 복원돼야 합니다.
...
| F12가 안 됨 | 노트북은 `Fn+F12`가 필요할 수 있음. 전용 바로가기로 열었는지 확인하고 `Ctrl+Alt+V`로 복원. |
| 미리보기 복원 시간 초과 | v1.2.0에는 제목 변경 수정 포함. “업데이트 후 저장하지 않은 파일을 보존하고 전용 창을 다시 열어 검증해줘.” |
