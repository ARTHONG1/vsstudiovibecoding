...
3. **Prepare.** Run `scripts/setup.ps1` without `-Apply` to obtain the concrete plan. Missing VS Code: install through verified official instructions within authorization. Missing OpenCode: `setup.ps1 -Apply` automatically attempts global npm installation if npm is present; otherwise install following official instructions. Run the discovered CLI's `--version`.
